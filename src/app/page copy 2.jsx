...
import {
  ...
  ChevronLeft, ChevronRight, ChevronsLeft, CxhevronsRight, Filter, Database, Image as ImageIcon, Cpu, UploadCloud, BarChart3, LineChart, Globe
} from "lucide-react";
import {
...
const translations = {
  ...
  defect_type: { en: "Defect Type", ar: "نوع العيب" },
...
const REALISTIC_PANELS_RAW = [
    ...
    // Use the user-provided IR panels for the gallery as well
    { id: 4001, type: "ir", serial_number: "SN-IR-R4-20240136", timestamp: new Date().toISOString(), image_url: "https://i.ibb.co/GQHd003P/Diode-Multi.png", status: "failed", health_score: 91.3, defects: [{ type: "Diode-Multi" }] },
    { id: 4002, type: "ir", serial_number: "SN-IR-R4-20240137", timestamp: new Date().toISOString(), image_url: "https://i.ibb.co/N6VPMqqj/Hot-Spot.png", status: "failed", health_score: 92.6, defects: [{ type: "Hot_Spot" }] },
    { id: 3004, type: "ir", serial_number: "SN-IR-R4-20240138", timestamp: new Date(new Date().setDate(new Date().getDate() - 8)).toISOString(), image_url: "https://user-images.githubusercontent.com/32656910/210141662-7901e138-ac7e-4074-b52a-9f57f607d738.png?raw=true", health_score: 99.8, status: "passed", defects: [] },
];
...
export default function HaseenApp() {
  ...
  const [mainSidebarCollapsed, setMainSidebarCollapsed] = useState(false); // For desktop collapse
  ...
  return (
    <LanguageContext.Provider value={{ lang, t, dateLocale }}>
      <div className="flex h-screen overflow-hidden text-sm text-gray-300 bg-[#0F172A] font-['Cairo'] relative" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        ...
        {sidebarOpen && <div onClick={() => setSidebarOpen(false)} className="fixed inset-0 bg-black/60 z-40 lg:hidden"></div>}

        <aside className={`bg-[#1E293B] ${lang === 'ar' ? 'border-l' : 'border-r'} border-gray-700 flex flex-col transition-all duration-300 fixed lg:relative inset-y-0 ${lang === 'ar' ? 'right-0' : 'left-0'} z-50 transform ${sidebarOpen ? 'translate-x-0' : (lang === 'ar' ? 'translate-x-full' : '-translate-x-full')} lg:translate-x-0 ${mainSidebarCollapsed ? "w-20" : "w-64"}`}>
          <div className={`pt-6 pb-4 px-4 border-b border-gray-700 flex items-center ${mainSidebarCollapsed ? 'justify-center' : 'justify-between'}`}>
  ...
  <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-sky-500 to-emerald-500 flex items-center justify-center flex-shrink-0">
    <img src="https://i.ibb.co/kgJMdpXt/Picture1.png" alt="Misbar Logo Icon" className="h-7 w-7 object-contain" style={{ filter: 'brightness(0) invert(1)' }} />
  </div>
  <div>
    <h2 className="font-bold text-xl text-white whitespace-nowrap">{t('system_name')}</h2>
    <p className="text-xs text-gray-400 whitespace-nowrap">{t('system_subtitle')}</p>
  </div>
</div>
  ...
  {/* <div className="w-10 h-10 flex items-center justify-center flex-shrink-0"> */}
    {/* <img src="https://i.ibb.co/kgJMdpXt/Picture1.png" alt="Misbar Logo" className="h-8 w-8 object-contain" style={{ filter: 'brightness(0) invert(1)' }} /> */}
  {/* </div> */}
  {/* <div> */}
    {/* <h2 className="font-bold text-xl text-white whitespace-nowrap">{t('system_name')}</h2> */}
    {/* <p className="text-xs text-gray-400 whitespace-nowrap">{t('system_subtitle')}</p> */}
  {/* </div> */}
{/* </div> */}
              <button onClick={() => setMainSidebarCollapsed(!mainSidebarCollapsed)} className="p-2 hover:bg-gray-700 rounded-lg transition-colors flex-shrink-0 hidden lg:block">
                  {mainSidebarCollapsed ? (lang === 'ar' ? <ChevronsLeft size={20} className="text-gray-400" /> : <ChevronsRight size={20} className="text-gray-400" />) : <Menu size={20} className="text-gray-400" />}
              </button>
          </div>
        ...
        <section className="flex-1 flex flex-col min-w-0">
          <header className="h-20 px-4 sm:px-6 flex items-center justify-between border-b border-gray-700 bg-[#1E293B] flex-shrink-0">
            <div className="flex items-center gap-4">
              <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-2 -ml-2 text-gray-400">
                <Menu size={24} />
              </button>
              <div>
                <h1 className="text-lg sm:text-xl font-bold text-white">{PAGE_INFO[currentPage]?.main || t('system_name')}</h1>
                {PAGE_INFO[currentPage]?.sub && ( <p className="text-xs sm:text-sm font-normal text-gray-400">{PAGE_INFO[currentPage].sub}</p> )}
              </div>
            </div>

            {/* ===== START: ADDED LOGOS ===== */}
            <div className="hidden lg:flex items-center justify-center gap-8">
                {/* The "filter" style is added to make the dark text in the logos appear white */}
                <img src="https://i.ibb.co/gLyL0X55/Picture1.png" alt="Desert Technologies Logo" className="h-12 object-contain" style={{ filter: 'brightness(0) invert(1)' }} />
                <img src="https://i.ibb.co/23b3hNkz/image.png" alt="Industrial Hackathon Logo" className="h-14 object-contain" style={{ filter: 'brightness(0) invert(1)' }} />
                <img src="https://upload.wikimedia.org/wikipedia/ar/thumb/0/04/%D8%B5%D9%86%D8%AF%D9%88%D9%82_%D8%A7%D9%84%D8%AA%D9%86%D9%85%D9%8A%D8%A9_%D8%A7%D9%84%D8%B5%D9%86%D8%A7%D8%B9%D9%8A%D8%A9_%D8%A7%D9%84%D8%B3%D8%B9%D9%88%D8%AF%D9%8A.svg/2560px-%D8%B5%D9%86%D8%AF%D9%88%D9%82_%D8%A7%D9%84%D8%AA%D9%86%D9%85%D9%8A%D8%A9_%D8%A7%D9%84%D8%B5%D9%86%D8%A7%D8%B9%D9%8A%D8%A9_%D8%A7%D9%84%D8%B3%D8%B9%D9%88%D8%AF%D9%8A.svg.png" alt="SIDF Logo" className="h-12 object-contain" style={{ filter: 'brightness(0) invert(1)' }} />
            </div>
            {/* ===== END: ADDED LOGOS ===== */}

            {/* <div className="hidden lg:flex items-center justify-center gap-8"> */}
                {/* The "drop-shadow" filter is added to create a white glow, making the text readable while keeping original colors */}
                {/* <img src="https://i.ibb.co/gLyL0X55/Picture1.png" alt="Desert Technologies Logo" className="h-12 object-contain" style={{ filter: 'drop-shadow(0 0 2px rgba(255,255,255,0.8))' }} /> */}
                {/* <img src="https://i.ibb.co/23b3hNkz/image.png" alt="Industrial Hackathon Logo" className="h-14 object-contain" style={{ filter: 'drop-shadow(0 0 2px rgba(255,255,255,0.8))' }} /> */}
                {/* <img src="https://upload.wikimedia.org/wikipedia/ar/thumb/0/04/%D8%B5%D9%86%D9%88%D9%82_%D8%A7%D9%84%D8%AA%D9%86%D9%85%D9%8A%D8%A9_%D8%A7%D9%84%D8%B5%D9%86%D8%A7%D8%B9%D9%8A%D8%A9_%D8%A7%D9%84%D8%B3%D8%B9%D9%88%D8%AF%D9%8A.svg/2560px-%D8%B5%D9%86%D9%88%D9%82_%D8%A7%D9%84%D8%AA%D9%86%D9%85%D9%8A%D8%A9_%D8%A7%D9%84%D8%B5%D9%86%D8%A7%D8%B9%D9%8A%D8%A9_%D8%A7%D9%84%D8%B3%D8%B9%D9%88%D8%AF%D9%8A.svg.png" alt="SIDF Logo" className="h-10 object-contain" style={{ filter: 'drop-shadow(0 0 2px rgba(255,255,255,0.8))' }} /> */}
            {/* </div> */}

            <div className="flex items-center gap-2 sm:gap-4">
              <button onClick={toggleLang} className="w-10 h-10 flex items-center justify-center bg-transparent hover:bg-gray-700 rounded-lg transition-colors text-sm font-bold text-gray-300 border border-gray-700 hover:border-gray-600" aria-label="Change language">
                {lang === 'ar' ? 'EN' : 'AR'}
              </button>
              ...
              <div className="flex items-center gap-3 cursor-pointer hover:bg-gray-700/50 rounded-lg p-2 transition-colors">
                <div className="w-9 h-9 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center">
                  <span className="text-white text-sm font-bold">{lang === 'ar' ? 'م' : 'O'}</span>
                </div>
                <div className="hidden md:block">
                  <p className="font-semibold text-white">{t('operator_name')}</p>
                  <p className="text-xs text-gray-400">{t('operator_role')}</p>
                </div>
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 bg-[#0F172A]">
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
                <div className="flex items-center gap-2 sm:gap-4 flex-wrap">
                    ...
                    <div className="flex items-center gap-2 px-4 py-2 bg-[#1E293B] rounded-xl border border-gray-600 w-full sm:w-auto min-w-0 sm:min-w-[280px]"><Calendar size={16} className="text-gray-400" /><span className="font-medium text-white truncate">{dateDisplay}</span></div>
                ...
                <button onClick={() => refetch()} className="p-2 bg-[#1E293B] rounded-lg border border-gray-600 hover:bg-gray-700 transition-colors self-end lg:self-center"><RefreshCw size={18} className="text-gray-400" /></button>
            </div>
            <div className="grid gap-4 sm:gap-6 grid-cols-2 md:grid-cols-3 lg:grid-cols-5 mb-6"><StatCard title={t('stat_total')} value={data?.stats?.total_panels.toLocaleString() || 0} subtext={t('solar_panel_unit')} icon={<Activity size={24} className="text-blue-400" />} color="text-white"/><StatCard title={t('stat_passed')} value={data?.stats?.passed_panels.toLocaleString() || 0} subtext={t('panel_unit')} icon={<CheckCircle size={24} className="text-green-400" />} color="text-green-400"/><StatCard title={t('stat_failed')} value={data?.stats?.failed_panels.toLocaleString() || 0} subtext={t('panel_unit')} icon={<XCircle size={24} className="text-red-400" />} color="text-red-400"/><StatCard title={t('stat_conformance')} value={`${Number(data?.stats?.conformance_rate || 0).toFixed(1)}%`} subtext={t('passed_production_ratio')} icon={<TrendingUp size={24} className="text-cyan-400" />} color="text-cyan-400"/><StatCard title={t('stat_defect_rate')} value={`${Number(data?.stats?.defect_rate || 0).toFixed(1)}%`} subtext={t('failed_production_ratio')} icon={<AlertTriangle size={24} className="text-orange-400" />} color="text-orange-400"/></div>
            <div className="flex justify-center mb-6">
                <div className="w-full max-w-2xl bg-[#1E293B] rounded-2xl border border-gray-700 p-2 flex flex-col sm:flex-row items-stretch">
                    <div className="flex-1 flex items-center justify-between bg-slate-900/50 rounded-t-lg sm:rounded-s-lg sm:rounded-tr-none px-4 sm:px-6 py-4">
                        <span className="text-xl sm:text-2xl font-bold text-white">{Number(data?.stats?.throughput_per_hour || 0).toFixed(1)}</span>
                        <div className="flex items-center gap-2 sm:gap-3 text-gray-300 text-right"><Gauge size={20} /><span className="text-sm sm:text-base">{t('throughput_per_hour')}</span></div>
                    </div>
                    <div className="flex-1 flex items-center justify-between bg-slate-900/50 rounded-b-lg sm:rounded-e-lg sm:rounded-bl-none px-4 sm:px-6 py-4 border-t sm:border-t-0 sm:border-s border-slate-700">
                        <span className="text-xl sm:text-2xl font-bold text-purple-400">97.4%</span>
                        <div className="flex items-center gap-2 sm:gap-3 text-gray-300 text-right"><Cpu size={20} className="text-purple-400" /><span className="text-sm sm:text-base">{t('model_accuracy')}</span></div>
                    </div>
                </div>
            </div>
            <div className="grid gap-6 grid-cols-1 lg:grid-cols-2"><div className="bg-[#1E293B] rounded-2xl p-4 sm:p-6 border border-gray-700"><h3 className="text-lg font-semibold text-white mb-4">{t('production_overview')}</h3><div className="h-80 sm:h-96"><ResponsiveContainer width="100%" height="100%"><AreaChart data={data?.productionData} margin={{ top: 5, right: lang === 'ar' ? 20 : 0, left: lang === 'ar' ? -15 : 10, bottom: 5 }}><defs><linearGradient id="colorInspected" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#38BDF8" stopOpacity={0.8}/><stop offset="95%" stopColor="#38BDF8" stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke="#374151" /><XAxis dataKey="date" tick={{ fontSize: 9, fill: "#9CA3AF" }} /><YAxis tick={{ fontSize: 9, fill: "#9CA3AF" }} /><Tooltip contentStyle={{ backgroundColor: "#1F2937", border: "1px solid #374151", borderRadius: "8px" }} /><Area type="monotone" dataKey="inspected" name={t('inspected_chart')} stroke="#38BDF8" fillOpacity={1} fill="url(#colorInspected)" /><Area type="monotone" dataKey="defects" name={t('defective_chart')} stroke="#EF4444" fill="transparent" /></AreaChart></ResponsiveContainer></div></div><div className="bg-[#1E293B] rounded-2xl p-4 sm:p-6 border border-gray-700 flex flex-col"><div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-2"><h3 className="text-lg font-semibold text-white">{t('defect_analysis_center')}</h3><div className="flex items-center bg-[#0F172A] rounded-lg border border-gray-600 p-1 self-start sm:self-center"><button onClick={() => setDefectView('pareto')} className={`px-2 py-1 rounded-md text-xs transition-colors flex items-center gap-1 ${defectView === 'pareto' ? 'bg-blue-600 text-white' : 'hover:bg-gray-700'}`}><LineChart size={14}/> <span>{t('impact')}</span></button><button onClick={() => setDefectView('full')} className={`px-2 py-1 rounded-md text-xs transition-colors flex items-center gap-1 ${defectView === 'full' ? 'bg-blue-600 text-white' : 'hover:bg-gray-700'}`}><BarChart3 size={14}/> <span>{t('all')}</span></button></div></div><div className="flex-1 h-80 sm:h-96">{data?.fullDefectData && data.fullDefectData.length > 0 ? ( <ResponsiveContainer width="100%" height="100%">{defectView === 'pareto' ? ( <ComposedChart data={data.paretoData} margin={{ top: 5, right: lang === 'ar' ? 5 : 25, left: lang === 'ar' ? -25 : 5, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" stroke="#374151" /><XAxis dataKey="name" tick={{ fontSize: 8, fill: "#9CA3AF" }} angle={-30} textAnchor="end" height={40} /><YAxis yAxisId="left" tick={{ fontSize: 9, fill: "#9CA3AF" }} orientation={lang === 'ar' ? 'left' : 'right'} /><YAxis yAxisId="right" orientation={lang === 'ar' ? "right" : "left"} tickFormatter={(tick) => `${tick}%`} tick={{ fontSize: 9, fill: "#f97316" }} /><Tooltip contentStyle={{ backgroundColor: "#1F2937", border: "1px solid #374151", borderRadius: "8px" }} /><Bar yAxisId="left" dataKey="count" name={t('count_chart')} fill="#38BDF8" /><Line yAxisId="right" type="monotone" dataKey="cumulativePercentage" name={t('cumulative_perc_chart')} stroke="#f97316" strokeWidth={2} dot={{ r: 3 }} /></ComposedChart> ) : ( <BarChart layout="vertical" data={data.fullDefectData} margin={{ top: 5, right: lang === 'ar' ? 20 : 10, left: lang === 'ar' ? 10 : 20, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" stroke="#374151" /><XAxis type="number" tick={{ fontSize: 9, fill: "#9CA3AF" }} /><YAxis type="category" dataKey="name" width={lang === 'ar' ? 80 : 120} tick={{ fontSize: 9, fill: "#9CA3AF" }} /><Tooltip contentStyle={{ backgroundColor: "#1F2937", border: "1px solid #374151", borderRadius: "8px" }} cursor={{ fill: '#374151' }}/><Bar dataKey="count" name={t('count_chart')}>{data.fullDefectData.map((entry, index) => ( <Cell key={`cell-${index}`} fill={entry.color} /> ))}</Bar></BarChart> )}</ResponsiveContainer> ) : <p className="text-sm text-gray-500 text-center py-4">{t('no_defect_data')}</p>}</div></div></div>
        </main>
    );
}
...
const IR_USER_PANELS = [
    { id: 4001, type: "ir", serial_number: "SN-IR-R4-20240136", timestamp: new Date().toISOString(), image_url: "https://i.ibb.co/GQHd003P/Diode-Multi.png", status: "failed", health_score: 91.3, defects: [{ type: "Diode-Multi" }] },
    { id: 3004, type: "ir", serial_number: "SN-IR-R4-20240137", timestamp: new Date().toISOString(), image_url: "https://user-images.githubusercontent.com/32656910/210141662-7901e138-ac7e-4074-b52a-9f57f607d738.png?raw=true", status: "passed", health_score: 99.5, defects: [] },
    { id: 4002, type: "ir", serial_number: "SN-IR-R4-20240138", timestamp: new Date().toISOString(), image_url: "https://i.ibb.co/N6VPMqqj/Hot-Spot.png", status: "failed", health_score: 92.6, defects: [{ type: "Hot_Spot" }] },
];
...
function ProductionLinePage({ lineType }) {
    ...
    const activePanels = useMemo(() => {
        if (lineType === 'ir') {
            return IR_USER_PANELS;
        }
        return REALISTIC_PANELS.filter(p => p.type === lineType);
    }, [lineType]);
    
    const currentPanel = activePanels[panelIndex];

    useEffect(() => {
        setPanelIndex(0); setAlerts([]);
        const firstPanel = activePanels[0];
        if (firstPanel?.status === 'failed') {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            setIsFading(true);
            setTimeout(() => {
                setPanelIndex(prevIndex => (prevIndex + 1) % activePanels.length);
                setIsFading(false);
            }, 500); 
        }, 3000); 
    ...
    useEffect(() => {
      if (!currentPanel || isFading) return;
      if (currentPanel.status === 'failed') {
          setAlerts(prevAlerts => {
              if (!prevAlerts.some(a => a.serial_number === currentPanel.serial_number)) {
                  const message = lang === 'ar' ? `${t('num_defects_detected')} ${currentPanel.defects.length} ${t('defects_detected')}` : `${currentPanel.defects.length} ${t('defects_detected')}`;
                  const newAlert = { id: currentPanel.id + Math.random(), serial_number: currentPanel.serial_number, message, timestamp: new Date().toISOString(), acknowledged: false };
                  return [newAlert, ...prevAlerts].slice(0, 50);
              }
              return prevAlerts;
          });
      }
    }, [currentPanel, isFading, lang, t]);
    
    const acknowledgeAlert = (id) => { setAlerts(prev => prev.map(a => a.id === id ? { ...a, acknowledged: true } : a)); };

    return (
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
             <main className="flex-1 flex flex-col p-4 sm:p-6 bg-[#0F172A] gap-6 overflow-hidden lg:order-2">
                <div className="flex-1 bg-[#1E293B] rounded-2xl p-2 sm:p-4 border border-gray-700 flex flex-col min-h-0">
                    <div className="flex-1 w-full flex items-center justify-center min-h-0 py-2">
                        <div className="h-full w-full max-w-full aspect-video mx-auto bg-black rounded-lg relative overflow-hidden shadow-2xl shadow-black/50">
                            {currentPanel ? ( <>
                                ...
                                {lineType === 'el' && currentPanel.defects.map((defect, i) => {
                                    const defectInfo = ALL_DEFECT_MAP[defect.type];
                                    if(!defectInfo) return null;
                                    return (<div key={i} className="absolute border rounded-sm sm:border-2" style={{ borderColor: defectInfo.color, boxShadow: `0 0 15px ${defectInfo.color}`, left: `${defect.location.x}%`, top: `${defect.location.y}%`, width: `${defect.location.w}%`, height: `${defect.location.h}%`, opacity: isFading ? 0 : 1, transition: 'opacity 0.5s' }}>
                                      <span className={`absolute ${lang === 'ar' ? 'right-0' : 'left-0'} -top-4 sm:-top-6 text-[7px] sm:text-xs px-1 sm:px-2 py-0 sm:py-0.5 rounded-md font-semibold text-white whitespace-nowrap`} style={{ backgroundColor: defectInfo.color }}>{defectInfo[lang]}</span>
                                    </div>);
                                })}
                    ...
                    {currentPanel && (
                      <>
                        {lineType === 'ir' ? (
                          <div className={`mt-4 bg-[#0F172A] border border-gray-700 rounded-xl p-3 grid grid-cols-2 md:grid-cols-5 gap-2 sm:gap-4 text-center w-full max-w-5xl mx-auto transition-opacity duration-500 flex-shrink-0 ${isFading ? 'opacity-0' : 'opacity-100'}`}>
                              <div><p className="text-[10px] sm:text-xs text-gray-400">{t('serial_number')}</p><p className="font-mono text-white text-xs sm:text-base break-all">{currentPanel.serial_number}</p></div>
                              <div><p className="text-[10px] sm:text-xs text-gray-400">{t('inspection_time')}</p><p className="font-mono text-white text-xs sm:text-base">{new Date().toLocaleTimeString(lang === 'ar' ? 'ar-SA' : 'en-US')}</p></div>
                              <div><p className="text-[10px] sm:text-xs text-gray-400">{t('health_score')}</p><p className={`font-bold text-base sm:text-xl ${currentPanel.status === 'passed' ? 'text-green-400' : 'text-red-400'}`}>{currentPanel.health_score.toFixed(1)}%</p></div>
                              <div><p className="text-[10px] sm:text-xs text-gray-400">{t('result')}</p><p className={`font-bold text-base sm:text-xl ${currentPanel.status === 'passed' ? 'text-green-400' : 'text-red-400'}`}>{currentPanel.status === 'passed' ? t('passed') : t('failed')}</p></div>
                              <div><p className="text-[10px] sm:text-xs text-gray-400">{t('defect_type')}</p>{currentPanel.status === 'passed' ? <p className="font-bold text-base sm:text-xl text-green-400">---</p> : <p className="font-mono text-orange-400 text-xs sm:text-base break-words">{currentPanel.defects[0]?.type.replace(/_/g, ' ') || '---'}</p>}</div>
                          </div>
                        ) : (
                          <div className={`mt-4 bg-[#0F172A] border border-gray-700 rounded-xl p-3 grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-4 text-center w-full max-w-4xl mx-auto transition-opacity duration-500 flex-shrink-0 ${isFading ? 'opacity-0' : 'opacity-100'}`}>
                              <div><p className="text-[10px] sm:text-xs text-gray-400">{t('serial_number')}</p><p className="font-mono text-white text-xs sm:text-base break-all">{currentPanel.serial_number}</p></div>
                              <div><p className="text-[10px] sm:text-xs text-gray-400">{t('inspection_time')}</p><p className="font-mono text-white text-xs sm:text-base">{new Date().toLocaleTimeString(lang === 'ar' ? 'ar-SA' : 'en-US')}</p></div>
                              <div><p className="text-[10px] sm:text-xs text-gray-400">{t('health_score')}</p><p className={`font-bold text-base sm:text-xl ${currentPanel.status === 'passed' ? 'text-green-400' : 'text-red-400'}`}>{currentPanel.health_score.toFixed(1)}%</p></div>
                              <div><p className="text-[10px] sm:text-xs text-gray-400">{t('result')}</p><p className={`font-bold text-base sm:text-xl ${currentPanel.status === 'passed' ? 'text-green-400' : 'text-red-400'}`}>{currentPanel.status === 'passed' ? t('passed') : t('failed')}</p></div>
             ...
             <aside className={`w-full lg:w-80 bg-[#1E293B] ${lang === 'ar' ? 'border-l lg:border-r-0' : 'border-r lg:border-l-0'} border-gray-700 flex flex-col flex-shrink-0 h-1/3 lg:h-full lg:order-1`}>
                <h3 className="text-base sm:text-lg font-semibold text-white p-4 sm:p-6 border-b border-gray-700 flex-shrink-0">{t('realtime_alerts')}</h3>
...
const GalleryCard = ({ panel }) => {
  const { lang, t } = useLang();
  return ( <div className="bg-[#1E293B] rounded-2xl p-4 border border-gray-700 flex flex-col gap-4">
    <div className="aspect-video bg-black rounded-lg relative overflow-hidden">
        <img src={panel.image_url} alt={`Panel ${panel.serial_number}`} className="h-full w-full object-contain" />
        {panel.type === 'el' && panel.defects.map((defect, i) => {
            const defectInfo = ALL_DEFECT_MAP[defect.type];
            return (<div key={i} className="absolute border rounded-sm opacity-90" style={{ borderColor: defectInfo?.color, boxShadow: `0 0 8px ${defectInfo?.color}`, left: `${defect.location.x}%`, top: `${defect.location.y}%`, width: `${defect.location.w}%`, height: `${defect.location.h}%` }}></div>)
        })}
        <span className={`absolute top-2 ${lang === 'ar' ? 'right-2' : 'left-2'} text-xs font-bold px-2 py-1 rounded-full text-white ${panel.type === 'el' ? 'bg-sky-500/80' : 'bg-rose-500/80'}`}>{panel.type.toUpperCase()}</span>
    </div>
    <div>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-2 gap-2">
            <p className="font-mono text-sm sm:text-base text-blue-400 break-all">{panel.serial_number}</p>
            <div className="flex items-center gap-1 text-xs text-gray-400 flex-shrink-0"><Calendar size={14} /><span>{new Date(panel.timestamp).toLocaleString(lang === 'ar' ? 'ar-SA' : 'en-US', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</span></div>
        </div>
        <div className="flex justify-between items-center gap-4 pt-3 border-t border-gray-700/50">
            <div className="text-center flex-1"><p className="text-xs text-gray-400">{t('result')}</p><p className={`font-bold text-lg ${panel.status === 'passed' ? 'text-green-400' : 'text-red-400'}`}>{panel.status === 'passed' ? t('passed') : t('failed')}</p></div>
            {panel.type === 'ir' && 
                <div className="text-center flex-1">
                    <p className="text-xs text-gray-400">{t('defect_type')}</p>
                    {panel.status === 'passed' ? <p className="font-bold text-lg text-green-400">---</p> : <p className="font-mono text-orange-400 text-sm break-words">{panel.defects[0]?.type.replace(/_/g, ' ') || '---'}</p>}
                </div>
            }
            <div className="text-center flex-1"><p className="text-xs text-gray-400">{t('health_score')}</p><p className={`font-bold text-lg ${panel.status === 'passed' ? 'text-green-400' : 'text-red-400'}`}>{panel.health_score.toFixed(1)}%</p></div>
            <div className="text-center flex-1"><p className="text-xs text-gray-400">{t('defects_count')}</p><p className="font-bold text-lg text-white">{panel.defects.length}</p></div>
        </div>
    </div>
  </div> );
};
...
function GalleryPage() {
    ...
    return (
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 bg-[#0F172A]">
             ...
             {filteredPanels.length > 0 ? ( <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">{filteredPanels.map(panel => ( <GalleryCard key={panel.id} panel={panel} /> ))}</div> ) : ( <div className="text-center py-20"><Search size={48} className="text-gray-500 mx-auto mb-4" /><h3 className="text-xl font-semibold text-white">{t('no_results_found')}</h3><p className="text-gray-400 mt-2">{t('try_modifying_search')}</p></div> )}
        </main>
    );
}

const PREDEFINED_DEFECTS_SET_1 = [ 
  { type: 'cell_crack', location: { x: 68.6, y: 36, w: 6.8, h: 13.5 } }, 
  { type: 'cell_crack', location: { x: 94, y: 23.3, w: 6.3, h: 13 } }, 
  { type: 'cell_crack', location: { x: 87.8, y: 62.3, w: 6.8, h: 13.2 } }, 
  { type: 'cell_crack', location: { x: 12, y: 12.33, w: 6, h: 13.5 } }, 
  { type: 'cell_crack', location: { x: 0, y: 62.7, w: 5.8, h: 13 } }, 
  { type: 'cell_crack', location: { x: 0, y: 13, w: 5.8, h: 13} }, 
  { type: 'cell_crack', location: { x: 94.3, y: 75, w: 5.8, h: 13 } }, 
  { type: 'cell_crack', location: { x: 55.6, y: 62, w: 6.2, h: 13.3 } }, 
  { type: 'cell_crack', location: { x: 5.83, y: 12.7, w: 6, h: 13.1 } }, 
  { type: 'cell_crack', location: { x: 30, y: 37, w: 6.3, h: 13.3 } },
   { type: 'cell_crack', location: { x: 81.6, y: 49, w: 6.4, h: 13.5 } }, 
   { type: 'cell_crack', location: { x: 94.3, y: 49, w: 6, h: 13.5 } }, 
   { type: 'cell_crack', location: { x: 12, y: 12, w: 6, h: 13.5 } },
    { type: 'cell_crack', location: { x: 55.8, y: 23.2, w: 6.3, h: 13.3 } },
     { type: 'cell_crack', location: { x: 11.75, y: 25, w: 6.2, h: 13.3 } }, // --------
      { type: 'cell_crack', location: { x: 11.5, y: 50, w: 6, h: 13.3 } }, 
      { type: 'cell_crack', location: { x: 55.5, y: 75, w: 6.2, h: 13.2 } }, // ------------- 
      { type: 'connector_corrosion', location: { x: 81.6, y: 49, w: 6.3, h: 13.3 } }, 
      { type: 'connector_corrosion', location: { x: 36.17, y: 75, w: 6.3, h: 13.3 } },
       { type: 'connector_corrosion', location: { x: 36.5, y: 11, w: 6.3, h: 13.3 } }, 
       { type: 'connector_corrosion', location: { x: 55.83, y: 0, w: 6.3, h: 11 } } 
      ];
...
function AIModelTestPage() {
    ...
    return (
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 bg-[#0F172A]">
            {!imageUrl ? ( <div onDrop={handleDrop} onDragOver={handleDragOver} className="h-full min-h-[calc(100vh-10rem)] flex flex-col items-center justify-center border-2 border-dashed border-gray-600 rounded-2xl text-center p-8"><UploadCloud size={64} className="text-gray-500 mb-4"/><h2 className="text-xl font-semibold text-white mb-2">{t('drop_image_title')}</h2><p className="text-gray-400 mb-6">{t('drop_image_subtitle')}</p><input type="file" accept="image/*" ref={fileInputRef} onChange={(e) => handleFileChange(e.target.files[0])} className="hidden"/><button onClick={() => fileInputRef.current.click()} className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-6 rounded-lg transition-colors">{t('select_file')}</button>{error && <p className="text-red-400 mt-4">{error}</p>}</div> ) : (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 bg-[#1E293B] rounded-2xl p-4 sm:p-6 border border-gray-700 flex items-center justify-center min-h-[300px]"><div className="w-full h-full relative"><img src={imageUrl} alt="Uploaded Panel" className="w-full h-full object-contain max-h-[calc(100vh-14rem)]" />{results && results.defects.map((defect, i) => {
                        const defectInfo = ALL_DEFECT_MAP[defect.type];
                        if(!defectInfo) return null;
                        return (<div key={i} className="absolute border sm:border-2 rounded-sm" style={{ borderColor: defectInfo.color, boxShadow: `0 0 15px ${defectInfo.color}`, left: `${defect.location.x}%`, top: `${defect.location.y}%`, width: `${defect.location.w}%`, height: `${defect.location.h}%`, }}>
                            <span className={`absolute ${lang === 'ar' ? 'right-0' : 'left-0'} -top-4 sm:-top-6 text-[7px] sm:text-xs px-1 sm:px-2 py-0 sm:py-0.5 rounded-md font-semibold text-white whitespace-nowrap`} style={{ backgroundColor: defectInfo.color }}>{defectInfo[lang]}</span>
                        </div>)
                    })}</div></div>
                    <div className="bg-[#1E293B] rounded-2xl p-6 border border-gray-700 flex flex-col">
                        ...
                        {isProcessing ? ( <div className="flex-1 flex flex-col items-center justify-center text-center min-h-[200px]"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-sky-500 mb-4"></div><p className="text-gray-300">{t('analyzing_image')}</p><p className="text-xs text-gray-500">{t('analysis_time_notice')}</p></div> ) : results ? ( <div className="flex-1 flex flex-col"><div className="grid grid-cols-2 gap-4 mb-6"><div className="bg-[#0F172A] p-4 rounded-lg text-center"><p className="text-sm text-gray-400">{t('result')}</p><p className={`text-xl sm:text-2xl font-bold ${results.status === 'passed' ? 'text-green-400' : 'text-red-400'}`}>{results.status === 'passed' ? t('passed') : t('failed')}</p></div><div className="bg-[#0F172A] p-4 rounded-lg text-center"><p className="text-sm text-gray-400">{t('health_score')}</p><p className={`text-xl sm:text-2xl font-bold ${results.status === 'passed' ? 'text-green-400' : 'text-red-400'}`}>{results.health_score.toFixed(1)}%</p></div></div><h4 className="font-semibold text-white mb-2">{t('detected_defects')}:</h4><div className="flex-1 overflow-y-auto pr-2 max-h-52 sm:max-h-64">{results.defects.length > 0 ? ( (() => { const defectSummary = Object.values(results.defects.reduce((acc, defect) => { if (!acc[defect.type]) { const defectInfo = ALL_DEFECT_MAP[defect.type]; acc[defect.type] = { type: defectInfo?.[lang], color: defectInfo?.color, count: 0 }; } acc[defect.type].count++; return acc; }, {})); return ( <ul className="space-y-3 mt-2">{defectSummary.map((d, i) => ( <li key={i} className="flex justify-between items-center text-sm sm:text-base p-2 rounded-lg"><div className="flex items-center gap-3"><span className="font-medium text-gray-200">{d.type}</span><span className="w-3 h-3 rounded-full" style={{backgroundColor: d.color}}></span></div>{d.count > 1 && ( <span className="font-mono text-sm bg-slate-700 text-slate-300 px-2 py-1 rounded-lg">x{d.count}</span> )}</li> ))}</ul> ); })() ) : <p className="text-sm text-gray-500 mt-4">{t('no_defects_found')}</p>}</div></div> ) : ( <div className="flex-1 flex flex-col items-center justify-center text-center min-h-[200px]"><CheckCircle size={48} className="text-gray-500 mb-4"/><p className="text-gray-300">{t('image_ready')}</p><p className="text-xs text-gray-500">{t('click_start_analysis')}</p></div> )}
                        <div className="mt-auto pt-4 flex flex-col sm:flex-row gap-4"><button onClick={resetState} className="w-full bg-gray-600 hover:bg-gray-700 text-white font-semibold py-2 px-4 rounded-lg transition-colors">{t('test_another_image')}</button><button onClick={runAIAnalysis} disabled={isProcessing || !imageFile} className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-blue-800 disabled:cursor-not-allowed text-white font-semibold py-2 px-4 rounded-lg transition-colors flex items-center justify-center gap-2">{isProcessing ? t('analyzing') : t('start_analysis')}</button></div>
                    </div>
                </div>
            )}
        </main>
